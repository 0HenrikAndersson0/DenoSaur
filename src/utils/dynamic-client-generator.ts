...
`;
  ...
  for (const method of httpMethods) {
    ...
    for (const [resourceName, paths] of Object.entries(methodPaths)) {
...
function analyzePath(path: string, method: string, operation: any): PathInfo {
  ...
  const pathParams = path.match(/\{([^}]+)\}/g)?.map(param => param.slice(1, -1)) || [];
...
function groupPathsByResource(pathInfos: PathInfo[]): Record<string, Record<string, PathInfo[]>> {
...
function generateCollectionMethods(resourceName: string, pathInfo: PathInfo): string {
  ...
  if (pathInfo.method === 'get') {
    ...
    code += `      queryParams: async (params: QueryParams = {}): Promise<ApiResponse<${responseType}>> => {\n`;
    code += `        const url = new URL(\`\${this.config.baseUrl}${pathInfo.path}\`);\n`;
  ...
  if (pathInfo.operation.requestBody) {
    ...
    const responseType = getResponseType(pathInfo.operation);
    code += `      data: async (body: ${requestType}): Promise<ApiResponse<${responseType}>> => {\n`;
    code += `        const response = await fetch(\`\${this.config.baseUrl}${pathInfo.path}\`, {\n`;
    code += `          method: '${pathInfo.method.toUpperCase()}',\n`;
    code += `          headers: {\n`;
...
function generateResourceMethods(resourceName: string, pathInfo: PathInfo): string {
  ...
  if (pathInfo.method === 'get') {
    ...
    code += `        const response = await fetch(\`\${this.config.baseUrl}${pathInfo.path.replace('{' + paramName + '}', '${' + paramName + '}')}\`, {\n`;
    code += `          method: '${pathInfo.method.toUpperCase()}',\n`;
    code += `          headers: this.config.headers,\n`;
    code += `        });\n`;
    code += `        \n`;
...
function getResponseType(operation: any): string {
  ...
  if (schema.type === 'array') {
    const itemType = schema.items?.$ref ? schema.items.$ref.split('/').pop() : 'any';
    return `${itemType}[]`;
  }
